...
public static class FlightEngineKutta
{
    ...
    public static List<Waypoint> CalculateWaypoints(Aircraft aircraft, List<Interaction> interactions, Vector2 windVelocity, float deltaTime, int steps)
    {
        ...
        for (int i = 1; i < steps; i++)
        {
            ...
            for (int j = 1; j < 5; ++j)
            {
                ...
                /*
                k[0] = velocity;
                k[1] = velocity + (new Vector3(0f, resultingforce.y, resultingforce.x) * (float)(1 / aircraft.mass) * deltaTime/2.0)
                */
